...
function volumes3D(exo, u) {
  ...
  if (exo.decor === 'machine') machine3D(prim, exo, sq, u);
...
function boite3D(prim, x0, x1, y0, y1, z0, z1, top, front, side) {
  ...
  q([[x0, y1, z0], [x1, y1, z0], [x1, y1, z1], [x0, y1, z1]], top);   // dessus
  q([[x1, y1, z0], [x1, y1, z1], [x1, y0, z1], [x1, y0, z0]], front); // face avant
  q([[x0, y1, z1], [x1, y1, z1], [x1, y0, z1], [x0, y0, z1]], side);  // côté proche
}

/** Machine à ischios/quadriceps : assise ou tapis, rouleau aux chevilles
    (le levier qui bouge avec les jambes), bras de levier et colonne de poids. */
function machine3D(prim, exo, sq, u) {
  const cx = sq.bassin.x;
  ...
  const assis = exo.pattern === 'iso-quad';   // leg extension = assis ; sinon leg curl = allongé
  const topY = SOL - sq.bassin.y - 3;
  const zc = Z_HANCHE + 6;

  if (assis) {
    boite3D(prim, -16, 18, topY - 7, topY, -15, 15, '#5b636f', '#474e59', '#3a404a'); // siège
    boite3D(prim, -19, -15, topY, topY + 40, -15, 15, '#474e59', '#3f4650', '#333944'); // dossier
    boite3D(prim, -22, -18, 0, topY, 11, 15, '#333944', '#2c313a', '#262a31');           // montant
  } else {
    boite3D(prim, -40, 22, topY - 7, topY, -15, 15, '#5b636f', '#474e59', '#3a404a');    // tapis long
    for (const cxp of [-32, 14]) boite3D(prim, cxp - 3, cxp + 3, 0, topY - 7, 11, 15, '#333944', '#2c313a', '#262a31'); // pieds
  }

  // Rouleau matelassé aux chevilles + bras de levier (côté proche).
  prim.push({ k: 'os', a: [cheville[0], cheville[1], -zc], b: [cheville[0], cheville[1], zc], rA: 6.5, rB: 6.5, mat: 'coussin' });
  prim.push({ k: 'os', a: [genou[0], genou[1], zc], b: [cheville[0], cheville[1], zc], rA: 2, rB: 2, mat: 'metal' });

  // Colonne de poids sur le côté, qui monte avec la contraction.
  const xS = assis ? 6 : -8, zS = -(Z_HANCHE + 24);
  prim.push({ k: 'os', a: [xS, 6, zS], b: [xS, 78, zS], rA: 1.8, rB: 1.8, mat: 'metal' });
  const yb = 12 + u * 32;
  boite3D(prim, xS - 9, xS + 9, yb, yb + 18, zS - 7, zS + 7, '#8a929e', '#5c6572', '#3a3f48');
}
